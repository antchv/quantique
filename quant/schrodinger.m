...
if multiple_square_wells
    well_width = 0.5;
    well_spacing = 10;

    num_wells = 4;  % User input for the number of wells
    well_positions = linspace(0, (num_wells - 1) * well_spacing, num_wells);

    for well_position = well_positions
        well_start = max(1, round(well_position / step_size));  % Ensure well_start is at least 1
        well_end = min(number_of_data_points, round((well_position + well_width) / step_size));  % Ensure well_end is within bounds

        for i = well_start:well_end
            potential(i, i) = 1 + potential(i, i);  % Set the potential to a positive value for a potential barrier
        end
    end
end
